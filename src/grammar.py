...
class MultiDGrammar:
    ...
    def type_matches(self, candidate, tp):
        if tp == 'base':
            return isinstance(candidate, gpm.Kernel) and (not candidate.is_operator)
        elif tp == 'kernel':
            return isinstance(candidate, gpm.Kernel)
        elif tp == 'base-not-const':
            return isinstance(candidate, gpm.Kernel) and (not candidate.is_operator) and (not isinstance(candidate, gpm.ConstKernel))
        elif tp == 'dimension':
    ...
    def list_options(self, tp):
        if tp == 'base':
            return list(gpm.base_kernels(self.ndim, self.base_kernels))
        elif tp == 'kernel':
            raise RuntimeError("Cannot expand the '%s' type" % tp)
        elif tp == 'base-not-const':
            return [k for k in self.list_options(tp='base') if not isinstance(k, gpm.ConstKernel)]
        elif tp == 'dimension':
...
def replace_all(polish_expr, mapping):
    if type(polish_expr) == tuple:
        ...
        assert isinstance(polish_expr, gpm.Kernel)
...
def polish_to_kernel(polish_expr):
    if type(polish_expr) == tuple:
        if polish_expr[0] == '+':
            ...
            return gpm.SumKernel(operands)
        elif polish_expr[0] == '*':
            operands = [polish_to_kernel(e) for e in polish_expr[1:]]
            return gpm.ProductKernel(operands)
        elif polish_expr[0] == '*-const':
            operands = [polish_to_kernel(e) for e in polish_expr[1:]]
            return gpm.ProductKernel([operands[0], gpm.SumKernel([operands[1], gpm.ConstKernel()])])
        elif polish_expr[0] == 'CP':
            base_kernel = polish_to_kernel(polish_expr[2])
            return gpm.ChangePointKernel(dimension=polish_expr[1], operands=[base_kernel.copy(), base_kernel.copy()])
        elif polish_expr[0] == 'CW':
            base_kernel = polish_to_kernel(polish_expr[2])
            return gpm.ChangeWindowKernel(dimension=polish_expr[1], operands=[base_kernel.copy(), base_kernel.copy()])
        elif polish_expr[0] == 'B':
            base_kernel = polish_to_kernel(polish_expr[2])
            return gpm.ChangeWindowKernel(dimension=polish_expr[1], operands=[gpm.ConstKernel(), base_kernel.copy()])
        elif polish_expr[0] == 'BL':
            base_kernel = polish_to_kernel(polish_expr[2])
            return gpm.ChangeWindowKernel(dimension=polish_expr[1], operands=[base_kernel.copy(), gpm.ConstKernel()])
        elif polish_expr[0] == 'None':
            return gpm.NoneKernel()
        else:
            raise RuntimeError('Unknown operator: %s' % polish_expr[0])
    else:
        assert isinstance(polish_expr, gpm.Kernel) or (polish_expr is None) or (type(polish_expr) == int)
        if isinstance(polish_expr, gpm.Kernel):
            return polish_expr.copy()
        else:
            return polish_expr
...
def expand_single_tree(kernel, grammar):
    assert isinstance(kernel, gpm.Kernel)
    result = []
...
def expand_kernels(D, seed_kernels, base_kernels='SE', rules=None):    
    ...
    kernels = gpm.remove_duplicates(kernels)
    kernels = [k for k in kernels if not isinstance(k, gpm.NoneKernel)]
    return kernels
